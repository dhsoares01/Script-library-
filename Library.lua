...
local theme = {
    ...
    ScrollViewBackground = Color3.fromRGB(30, 30, 40),
    ScrollBar = Color3.fromRGB(80, 80, 90),
}

function Library:CreateWindow(name)
    local ScreenGui = Instance.new("ScreenGui", game:GetService("CoreGui"))
    ScreenGui.Name = name or "PremiumUILib"
    ...
    -- Ajuste para cantos arredondados apenas no topo
    local CornerAdjust = Instance.new("Frame", TitleBar)
    CornerAdjust.Size = UDim2.new(1, 0, 1, 8)
    CornerAdjust.Position = UDim2.new(0, 0, 0, 0)
    CornerAdjust.BackgroundColor3 = theme.Tab
    CornerAdjust.BorderSizePixel = 0
    CornerAdjust.ZIndex = 2

    -- Título
    local Title = Instance.new("TextLabel", CornerAdjust)
    Title.Size = UDim2.new(1, -40, 1, 0)
    Title.Position = UDim2.new(0, 10, 0, 0)
    Title.BackgroundTransparency = 1
    Title.Text = name or "Premium UI"
    Title.TextSize = 18
    ...
    -- Botão minimizar
    local BtnMinimize = Instance.new("TextButton", CornerAdjust)
    ...
    function window:CreateTab(tabName, icon)
        ...
        -- Alternar para a página ao clicar
        ...
        function tab:AddToggle(text, callback)
            ...
            ToggleBtn.MouseEnter:Connect(function()
                if not state then
                    TweenService:Create(ToggleBtn, TweenInfo.new(0.1), { BackgroundColor3 = Color3.fromRGB(50, 50, 60) }):Play()
                end
            end)
            ToggleBtn.MouseLeave:Connect(function()
                if not state then
                    TweenService:Create(ToggleBtn, TweenInfo.new(0.1), { BackgroundColor3 = theme.Tab }):Play()
                end
            end)
